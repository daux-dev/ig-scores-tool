$( () => {
    ...
    $.getJSON("/data", data => {
        ...
        if (parseInt(data.p1_score) >= parseInt(data.max_score)) {
            $(".p1_score").addClass("red");
            $(".p1_name").addClass("red");
        }
        if (parseInt(data.p2_score) >= parseInt(data.max_score)) {
            $(".p2_score").addClass("blu");
            $(".p2_name").addClass("blu");
        }

        $(".format_name").text("DECIDE BY " + data.max_score);
    });
    ...
    setInterval(() => {
        $.getJSON("/data", data => {
            ...
            if ($(".scroller").text() !== data.scroller) {
                animFadeRight(".scroller", data.scroller);
            }
            ...
            if (parseInt(data.p1_score) >= parseInt(data.max_score)) {
                ...
                $(".p1_score").removeClass("red");
                $(".p1_name").removeClass("red");
            }
            if (parseInt(data.p2_score) >= parseInt(data.max_score)) {
                $(".p2_score").addClass("blu");
                $(".p2_name").addClass("blu");
            } else {
                $(".p2_score").removeClass("blu");
                $(".p2_name").removeClass("blu");
            }

            if ($(".format_name").text() !== "DECIDE BY " + data.max_score) {
                animFadeDown(".format_name", "DECIDE BY " + data.max_score);
            }
        });
    }, 2000);
});
