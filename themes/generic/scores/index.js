$( () => {
    ...
    $.getJSON("/data", data => {
        ...
        $(".stage_desc").text(data.stage_desc);
    ...
    setInterval(() => {
        $.getJSON("/data", data => {
            ...
            if ($(".stage_desc").text() !== data.stage_desc) {
                animFadeDown(".stage_desc", data.stage_desc);
            }
            if ($(".commentary").text() !== data.commentary) {
                animFadeDown(".commentary", data.commentary);
                if (data.commentary == "") {
                    $(".commentaryBar").fadeOut(500);
                } else {
                    $(".commentaryBar").fadeIn(500);
                }
            }
            // if($(".commentary").text() !== "") {
            //     $(".commentaryBar").fadeIn(500);
